...
const createRecordSchema = z.object({
  ...
  if (data.serviceType === "Computer/Laptop Repair" && !data.deviceSerialNumber) {
    ctx.addIssue({
...
router.get("/", async (req: Request, res: Response) => {
  try {
    const { search, mobile, serviceType, printerModel, dateFrom, dateTo, sortBy } = req.query as Record<string, string>;
    ...
    if (search) {
      conditions.push(
        or(
          ilike(customerIntakeRecordsTable.firstName, `%${search}%`),
          ilike(customerIntakeRecordsTable.lastName, `%${search}%`),
          like(customerIntakeRecordsTable.mobileNumber, `%${search}%`),
          ilike(customerIntakeRecordsTable.printerModel, `%${search}%`),
        )
      );
    }
